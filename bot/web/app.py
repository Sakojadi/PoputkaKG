...
from bot.database.models import User, Campaign, AppSetting
...
async def dashboard(request: Request, _=Depends(require_admin)):
    async with AsyncSessionLocal() as session:
        ...
        if total_revenue == 0.0:
            tot_res = await session.execute(select(func.sum(Campaign.publications_total)))
            total_revenue = float(tot_res.scalar() or 0) * 1.0

        # Active campaigns
        act_res = await session.execute(
            select(func.count(Campaign.id)).where(Campaign.is_active == True, Campaign.publications_left > 0)
        )
...
async def campaigns_page(
    ...
    async with AsyncSessionLocal() as session:
        ...
        if q and q.strip():
            term = q.strip()
            if term.isdigit():
                filt = or_(Campaign.id == int(term), Campaign.user_id == int(term), Campaign.content_text.ilike(f"%{term}%"))
            else:
        ...
        res = await session.execute(
            base_query.order_by(desc(Campaign.id)).offset((current_page - 1) * limit).limit(limit)
        )
        ...
        if user_ids:
            ...
            for u in users_res.scalars().all():
                c_res = await session.execute(select(Campaign).where(Campaign.user_id == u.id))
...
async def admin_stop_campaign(campaign_id: int, _=Depends(require_admin)):
    ...
    return RedirectResponse(url="/admin/campaigns?msg=Объявление+успешно+остановлено", status_code=302)
...
async def admin_resume_campaign(campaign_id: int, _=Depends(require_admin)):
    async with AsyncSessionLocal() as session:
        ...
        if campaign and not campaign.is_active and campaign.publications_left > 0:
            ...
            await session.commit()
    return RedirectResponse(url="/admin/campaigns?msg=Объявление+возобновлено", status_code=302)
...
async def admin_delete_campaign(
            ...
            if delete_from_group:
                ...
                try:
                    ...
                    if campaign.message_ids:
                        msg_id_list.extend([int(x.strip()) for x in campaign.message_ids.split(",") if x.strip().isdigit()])
                    if campaign.last_message_id and campaign.last_message_id not in msg_id_list:
                        msg_id_list.append(campaign.last_message_id)

                    for m_id in set(msg_id_list):
                        try:
                            await bot.delete_message(chat_id=config.group_id, message_id=m_id)
                        except Exception as e:
                            logger.debug(f"Failed to delete message {m_id} from group: {e}")
                except Exception as e:
    ...
    msg = "Объявление+удалено+из+базы+и+стерто+из+группы" if delete_from_group else "Объявление+удалено+из+базы"
    ...
    async with AsyncSessionLocal() as session:
        ...
        if q and q.strip():
            ...
            if term.isdigit():
                ...
                filt = or_(User.username.ilike(f"%{term}%"), User.language.ilike(f"%{term}%"))
            base_query = base_query.where(filt)
            count_query = count_query.where(filt)
        ...
        usr_res = await session.execute(
            base_query.order_by(desc(User.id)).offset((current_page - 1) * limit).limit(limit)
        )
...
async def toggle_user_ban(
        ...
        if user:
            ...
            if user.is_banned:
                c_res = await session.execute(
                    select(Campaign).where(Campaign.user_id == user_id, Campaign.is_active == True)
                )
    ...
    return RedirectResponse(url=f"{target_url}{sep}msg=Статус+пользователя+в+боте+обновлен", status_code=302)

# --- BROADCAST & PROMO ---
@app.get("/admin/broadcast", response_class=HTMLResponse)
async def broadcast_page(request: Request, msg: Optional[str] = None, _=Depends(require_admin)):
    async with AsyncSessionLocal() as session:
...
async def broadcast_custom_group_post(
    request: Request,
    _=Depends(require_admin)
):
    ...
    if not text:
        return RedirectResponse(url="/admin/broadcast?msg=Текст+сообщения+не+может+быть+пустым", status_code=302)
...
async def broadcast_users(
    ...
    return RedirectResponse(url="/admin/broadcast?msg=Рассылка+запущена+в+фоновом+режиме!", status_code=302)

# --- SETTINGS ---
@app.get("/admin/settings", response_class=HTMLResponse)
async def settings_page(request: Request, msg: Optional[str] = None, _=Depends(require_admin)):
    return templates.TemplateResponse(
        request=request, 
        name="settings.html", 
        context={
...
async def settings_general_action(
    ...
    return RedirectResponse(url="/admin/settings?msg=Настройки+успешно+сохранены!", status_code=302)
